...
{
    public class MonthlyBudgetViewModel : ViewModelBase
    {
        ...
        // ===== TOTAL COMES FROM INCOME (BudgetPlan) =====
        ...
        public MonthlyBudgetViewModel(BudgetDbContext db, int userId)
        {
            _db = db;
            _userId = userId;
        ...
        public async Task ReloadAsync()
        {
            ...
            // NEW: load total monthly budget from BudgetPlan (Income)
            var plan = await _db.BudgetPlans
                .AsNoTracking()
                .FirstOrDefaultAsync(p => p.UserId == _userId && p.Month == month);
